...
// *************************************************************
public function UcitajSveMD($sortField = null, $where = null) {
    if (!isset($this->OtvorenaKonekcija->konekcijaDB)) {
        die("Nema validne mysqli konekcije!");
    }
    ...
    $result = mysqli_query($this->OtvorenaKonekcija->konekcijaDB, $SQL);

    if (!$result) {
        die("Greška SQL: " . mysqli_error($this->OtvorenaKonekcija->konekcijaDB));
    }
    ...
    $this->BrojZapisa = count($this->Kolekcija);

    return $this->Kolekcija; // moderni array
}
