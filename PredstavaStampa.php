...
{	
	...
	$PredstavaViewObject = new DBPredstava($KonekcijaObject,"Predstava");
	if (isset($_GET['filtriraj']))
		{
			// prikaz filtriranih podataka primenom pogleda nad kojim je dodat filter
			$filter=$_GET['filter'];
			$PredstavaViewObject->DajSvePodatkeOPredstavama($filter);
		...
		else
		{
			...
			$PredstavaViewObject->DajSvePodatkeOPredstavama($filter);
			// sada raspolazemo sa:
			//$PredstavaViewObject->Kolekcija 
			//$PredstavaViewObject->BrojZapisa
		}
}
else
{
